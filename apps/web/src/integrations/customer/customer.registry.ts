...
import type { SalesCustomer } from "./customer.mapper";
import { toSalesCustomer } from "./customer.mapper";
...
/**
 * Fetch all sales customers from the unified Contact Registry.
 * 
 * ARCHITECTURAL RATIONALE: 
 * Prevents the Sales module from maintaining an isolated, duplicate customer table.
 * All sales orders resolve customers directly from the shared Contact Registry via this ACL method.
 */
export async function getSalesCustomers(): Promise<SalesCustomer[]> {
  ...
  return contacts.map(toSalesCustomer);
}
